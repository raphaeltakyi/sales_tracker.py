...
if not df.empty:
    ...
    min_date = min(date_list) if date_list else datetime.now().date()
    max_date = max(date_list) if date_list else datetime.now().date()

    # Preset date ranges
    preset_options = {
        "Today": (datetime.now().date(), datetime.now().date()),
        "Last 7 Days": (max(datetime.now().date() - timedelta(days=6), min_date), datetime.now().date()),
        "This Month": (datetime.now().replace(day=1).date(), datetime.now().date()),
    ...
    start_preset, end_preset = preset_options[preset]
    ...
    # Unpack selected range
    if isinstance(selected_range, tuple):
        start_date, end_date = selected_range
    else:
        start_date, end_date = start_preset, end_preset
    ...
    # Apply filters with new date logic
